...
select nm_livro, vl_venda as 'Preco de venda', vl_venda + (vl_venda * 0.075) as 'Novo valor'
	from tb_livro
		where ano_publicacao < 1997;
        
select ano_publicacao, nm_livro
	from tb_livro
		where nm_livro like "O%";
        
select nm_autor, sobrenome_autor
	from tb_autor
		where nm_autor like "%S"
        and sobrenome_autor like "%S";

select nm_pais
	from tb_pais
		where nm_pais like "%R%";
        
select qt_estoque, char_length(isbn), nm_livro
	from tb_livro
		where nm_livro like "%de%";
        
select substring(nm_autor, 3,1), cd_autor
	from tb_autor
        where cd_autor between 5 and 16;
        
select nm_categoria, replace (nm_categoria, 'a', '&')
	from tb_categoria;
    
select avg(vl_venda) as media
	from tb_livro;

select cd_autor, cd_livro,
    count(*) as qtd
from tb_livro_autor group by cd_autor;

select min(vl_compra) as 'Menor valor', max(vl_compra) as 'Maior valor'
from tb_livro;

select sum( vl_compra) as 'Ativos'
from tb_livro;
