...
select nm_livro, ano_publicacao
	from tb_livro
		where ano_publicacao < '1999';
		
select qt_estoque, vl_compra
	from tb_livro
		where vl_compra < 30.00 and vl_compra > 20.00;
        
select nm_autor as Nome, sobrenome_autor as Sobrenome, cd_autor as Codigo
	from tb_autor
		where cd_autor in ('16','21');
	
select ano_publicacao, isbn
	from tb_livro
		where character_length(isbn) > 11;
        
select nm_pais, cd_pais
	from tb_pais
		where cd_pais not in ('3');
		
select tb_livro.qt_estoque, tb_livro.vl_compra, tb_livro.vl_venda, tb_livro.nm_livro, tb_pais.cd_pais
	from tb_livro join tb_pais
		where tb_livro.vl_venda > 40.00 and tb_pais.cd_pais not in ('4');
        
select nm_editora as Nome, cd_pais, cd_editora as Codigo
	from tb_editora
		where  cd_editora not in ('5') 
			and  cd_pais not in ('1','3');
            
select  nm_livro as Nome, cd_categoria as categoria, cd_editora as editora, isbn
	from tb_livro
		where cd_categoria >= 10 and cd_editora <= 3;
        
select cd_autor as 'Codigo Autor', cd_livro as 'CodigoLivro'
	from tb_livro_autor
		where cd_livro = ('25');
        
select nm_categoria as 'Categoria De Livros', cd_categoria
	from tb_categoria
		where cd_categoria between 3 and 8
			order by cd_categoria asc;
            
select cd_livro, ano_publicacao, qt_estoque, cd_editora
	from tb_livro
		where ano_publicacao >= '2006' and qt_estoque not in ('24') and cd_editora not in ('3')
			order by cd_editora desc;
